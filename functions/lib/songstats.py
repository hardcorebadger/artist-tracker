...
class SongstatsClient():
  ...
  def get_stat_weeks(self, spotify_id : str, end: datetime, weeks : int):
    res = self.get_historic_stats(spotify_id, end - timedelta(days=7*weeks), end)
    rollups = {}
    dates = []
    ...
    for source in res['stats']:
      ...
      for i, day in enumerate(source['data']['history']):
        # only look at the week ends
        d = datetime.strptime(day['date'], "%Y-%m-%d")
        ddiff = (end - d).days
        if ddiff != 0 and ddiff % 7 != 0:
          continue
        #  grab all the stats
        for key in day:
          if key == 'date':
